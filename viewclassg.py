...
import roman,sys,retrieve,view03g
class Ui_viewclass(object):
    ...
    def classviewing(self):
        romancheckk=retrieve.xx
        if romancheckk.isdigit()==True:
            self.classd.setText("  "+str(roman.toRoman(int(retrieve.xx)))+"  "+retrieve.yy.upper())
        elif romancheckk.isalpha()==True:
            self.classd.setText("  "+retrieve.xx.upper()+"  "+retrieve.yy.upper())
    ...
    def namesorting(self):
        ...
        for i in l:
            c+=i     
        retrieve.namesorting1(c.lower())
        for row_number,row_data in enumerate(retrieve.result1):
            self.tableView.insertRow(row_number)
            for column_number,column_data in enumerate(row_data):
                if column_number!=1:
...
"")
        ...
        self.label_4.setGeometry(QtCore.QRect(1270, 15, 47, 20))
        font = QtGui.QFont()
        font.setFamily("Palatino Linotype")
    ...
    def retranslateUi(self, viewclass):
        ...
        self.label_4.setText(_translate("viewclass", "guest"))
import pic
